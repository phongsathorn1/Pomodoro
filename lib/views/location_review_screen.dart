...
class LocationReviewScreen extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return LocationReviewScreenState();
  }
}

class LocationReviewScreenState extends State<LocationReviewScreen> {
  int _navIndex = 0;
  bool _isLoading;
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
        stream: Firestore.instance.collection("reviews_location").snapshots(),
        builder: (context, AsyncSnapshot<QuerySnapshot> snapshotReview) {
          ...
          if (snapshotReview.hasData) {
                                              ...
                                              child: CachedNetworkImage(
                                                ...
                                                imageUrl: snapshotUser
                                                    ...
                                                    ? snapshotUser
                                                        ...
                                                        .elementAt(0)
                                                        .data['photoUrl']
                                                    : 'http://serenityforge.com/template/wordpress/wp-content/uploads/2014/06/faceLocation-default-no-profile-pic.jpg',
                                                width: 55.0,
                                                height: 55.0,
                                                fit: BoxFit.cover,
                                              ),
            ...
            return Center(
              child: Text("ยังไม่มีรีวิวของสถานที่นี้"),
            );
          }
        });
  }
  ...
  setLike(String uid, String review_id, bool isLiked) {
    final DocumentReference reviewRef =
        Firestore.instance.document('reviews_location/${review_id}');
